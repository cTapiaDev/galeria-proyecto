...
const fetchImages = async () => {
  try {
    ...
    errorMessage.value = 'No se pudieron cargar las imágenes.';
  }
};

const handleFileChange = (event) => { newImageFile.value = event.target.files[0]; };
...
const handleSubmit = async () => {
  ...
  try {
    ...
    errorMessage.value = 'Hubo un error al subir la imagen.';
  } finally {
...
<template>
  <main class="container">
    ...
    <form @submit.prevent="handleSubmit" class="upload-form">
      ...
      <button type="submit" :disabled="isLoading">{{ isLoading ? 'Subiendo...' : 'Subir Imagen' }}</button>
